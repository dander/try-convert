    ...
    internal class Program
    {
        private static async Task<int> Main(string[] args)
        {
            ...
            var parser =
                ...
                .AddOption(new Option(new[] { "--keep-explicit-items" }, "Disable 'default item inclusion' to keep the existing explicit source references (useful when project are nested)") { Argument = new Argument<bool>(() => false) })
        ...
        public static int Run(string? project, string? workspace, string? msbuildPath, string? tfm, bool forceWebConversion, bool preview, bool diffOnly, bool noBackup, bool keepCurrentTfms, bool update, bool mauiConversion, bool forceRemoveCustomImports, bool keepExplicitItems)
        {
            if (update)
            {
            ...
            try
            {
                ...
                foreach (var item in msbuildWorkspace.WorkspaceItems)
                {
                    ...
                    {
                        var converter = new Converter(item.UnconfiguredProject, item.SdkBaselineProject, item.ProjectRootElement, noBackup, forceRemoveCustomImports, keepExplicitItems);
